...
## take away points in the middle bar (and changed it to dashes)
...
nomogrammer <- function(Prevalence,
  ...
  ## we'll also add the sig. level as 0.95
  sig.level=0.95
  alpha <- 1 - sig.level
  ...
  if(sensspec == TRUE){
    ...
    p1_plr = (sensitivity)
    p2_plr = (1-specificity)
    sigma2_PLR = ((1-p1_plr)/(p1_plr*n1)) + ((1-p2_plr)/(p2_plr*n2))
    lower.pos = exp(log(PLR) + (-qnorm(1-(alpha/2))*sqrt(sigma2_PLR)))
    upper.pos = exp(log(PLR) + (qnorm(1-(alpha/2))*sqrt(sigma2_PLR)))
    
    NLR <- (1-sensitivity)/specificity
    p1_nlr = (1-sensitivity)
    p2_nlr = (specificity)
    sigma2_NLR = ((1-p1_nlr)/(p1_nlr*n1)) + ((1-p2_nlr)/(p2_nlr*n2))
    lower.neg = exp(log(NLR) + (-qnorm(1-(alpha/2))*sqrt(sigma2_NLR)))
    upper.neg = exp(log(NLR) + (qnorm(1-(alpha/2))*sqrt(sigma2_NLR)))
    ...
    #95% CI for pos
    post_odds_pos_upper = prior_odds * upper.pos
    post_odds_pos_lower = prior_odds * lower.pos
    post_prob_pos_upper = post_odds_pos_upper/(1+post_odds_pos_upper)
    post_prob_pos_lower = post_odds_pos_lower/(1+post_odds_pos_lower)
    
    #95% CI for neg
    post_odds_neg_upper = prior_odds * upper.neg
    post_odds_neg_lower = prior_odds * lower.neg
    post_prob_neg_upper = post_odds_neg_upper/(1+post_odds_neg_upper)
    post_prob_neg_lower = post_odds_neg_lower/(1+post_odds_neg_lower)
  ...
  p <- ggplot(df) +
    ...
    geom_vline(xintercept = left+.001) +
    geom_vline(xintercept = right-.001) +
    ...
    annotate(geom="segment",
             x = rep(middle-.005, length(ticks_log_lrs)),
             xend = rep(middle+.008, length(ticks_log_lrs)),
             y = (ticks_log_lrs-scale_factor)/2,
             yend = (ticks_log_lrs-scale_factor)/2,
             size = 0.5) +
    # this is positive LR
    annotate(geom="segment",
             x = middle-.008, xend = middle+.01, y = (log10(lower.pos)-scale_factor)/2, 
             yend = (log10(lower.pos)-scale_factor)/2, size = 1.2, color = "red") +
    annotate(geom="segment",
             x = middle-.008, xend = middle+.01, y = (log10(upper.pos)-scale_factor)/2, 
             yend = (log10(upper.pos)-scale_factor)/2, size = 1.2, color = "red") + 
    
    # this is negative LR
    annotate(geom="segment",
             x = middle-.008, xend = middle+.01, y = (log10(lower.neg)-scale_factor)/2, 
             yend = (log10(lower.neg)-scale_factor)/2, size = 1.2, color = "blue") +
    annotate(geom="segment",
             x = middle-.008, xend = middle+.01, y = (log10(upper.neg)-scale_factor)/2, 
             yend = (log10(upper.neg)-scale_factor)/2, size = 1.2, color = "blue") +
    
    # this is positive post-test
    annotate(geom="segment",
             x = right-.008, xend = 1.01, y = (logodds(post_prob_pos_lower)), 
             yend = (logodds(post_prob_pos_lower)), size = 1.2, color = "red") +
    annotate(geom="segment",
             x = right-.008, xend = 1.01, y = (logodds(post_prob_pos_upper)), 
             yend = (logodds(post_prob_pos_upper)), size = 1.2, color = "red") + 
    
    # this is negative post-test
    annotate(geom="segment",
             x = right-.008, xend = 1.01, y = (logodds(post_prob_neg_lower)), 
             yend = (logodds(post_prob_neg_lower)), size = 1.2, color = "blue") +
    annotate(geom="segment",
             x = right-.008, xend = 1.01, y = (logodds(post_prob_neg_upper)), 
             yend = (logodds(post_prob_neg_upper)), size = 1.2, color = "blue") +
